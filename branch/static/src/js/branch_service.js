...
export const branchService = {
    ...
    start(env, { user, router, cookie }) {
        ...
        return {
            ...
            get currentBranch() {
                let def_branch = availableBranches[allowedBranchIds[0]]
                let currentCompanies = env.services.user.context.allowed_company_ids

                if (currentCompanies.includes(def_branch["company"])) {
                    console.log('if')
                    def_branch = availableBranches[allowedBranchIds[0]];
                } else {
                    console.log('else')
                    def_branch = false;
                    for (const [key, value] of Object.entries(availableBranches)) {
                        if (currentCompanies.includes(value["company"])) {
                            let branch = availableBranches[key];
                            def_branch = branch
                            break;
                        }
                    }
                }
                user.updateContext({ allowed_branch_ids: [def_branch['id']] });
                ajax.jsonRpc('/set_user_allowed_branch', 'call', {
                    'branch_id':  def_branch['id'],
                });
                return def_branch;
            },
